...
pcb_mainboard contains the [Teensy 4.1](https://www.pjrc.com/store/teensy41.html) microprocessor and connections for external components including the motor electronic speed controller (ESC), pressure sensor, the switches and LEDs, and the USB port. A Buck converter module regulates the voltage supplied to Teensy (5V). A coin cell battery holder powers Teensy’s real time clock (RTC) when the unit is powered down. Teensy’s built-in microSD card slot allows for data logging. Teensy’s microUSB adapter is connected to a USB-A connector on the Mainboard, allowing external connection to a PC via the bulkhead connector on the end cap. pcb_mainboard is mounted on the pcb_tray_part via four mounting holes.  Optional components include an INA219 current sensor (to monitor amps consumed by the motor) and a Hall effect flowmeter connection.
<P><table>
<tr>
<td width=355>
